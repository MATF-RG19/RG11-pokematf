...
//! Struct used for storing coordinates, width and height of objects in the
//! project.
...
//! Random value between min and max attack of the wild Pokemon player
//! encountered.
...
//! Function used to display text on the screen, x and y represent coordinates
//! of the text and s is the string that will be displayed.
...
//! Function initializes random pokemon min and max attack in predetermined
//! range.
...
//! Draws player on the first display using provided texture and position
//! determined by player movement.
...
//! Draws the red dot represening wild pokemon in the grass. Random position
//! after every battle.
static void draw_wild_pokemon();

//! Checks collision between two objects using their coordinates, width and
//! height. Returns bool value that determines if collision occurred.
static auto check_collision(float x1, float y1, float w1, float h1, float x2,
                            float y2, float w2, float h2) -> bool;

//! Checks if collision would occuring when player moved in any direction by
//! calling check_collision func.
static auto check_proximity(float x1, float y1, float w1, float h1, float x2,
                            float y2, float w2, float h2) -> bool;
...
//! Draws Pokemon that was previously selected with show_wild_pokemon variable.
//! Function has different ways of drawing a Pokemon depending on the display
//! mode.
static void draw_pokemons();

//! Draws Pokemon stats. Function has different ways of drawing a Pokemon
...
//! Light attack function that starts an attack animation and reduces opponents
//! health.
...
//! Heal pokemon. Function has two ways of working depending from which display
//! mode it was called.
...
static void heal_pokemon() {
  if (potion_charges <= 0) {
    ...
    if (poke_info[show_pokemon].health <= 0) {
      add_to_battle_log(
          "Pokemon is too tired to get up, heal him at Pokecenter");
      return;
    }
    potion_charges--;
...
static void light_attack() {
  if (turn) {
    ...
    favorite_current_attack =
        rand() % (poke_info[favorite_pokemon].attack_max + 1 -
                  poke_info[favorite_pokemon].attack_min) +
        poke_info[favorite_pokemon].attack_min;
    add_to_battle_log("Light attack " +
                      std::to_string(favorite_current_attack));
    turn = false;
    ...
    srand(time(nullptr));
    wild_current_attack = rand() % (wild_pokemon_stats.attack_max + 1 -
                                    wild_pokemon_stats.attack_min) +
                          wild_pokemon_stats.attack_min;
    add_to_battle_log("- " + std::to_string(wild_current_attack) +
                      "hp ~ wild pokemon light attack");
    turn = true;
...
static void catch_pokemon() {
  ...
  if (wild_pokemon_stats.health > 0 && wild_pokemon_stats.health <= 30 &&
      poke_info[favorite_pokemon].health > 0 && rand() % 2 == 0) {

    if (owned_pokemons.count(show_wild_pokemon)) {
      add_to_battle_log(
          "You already have this pokemon, sending it to Professor Oak...");
    } else {
...
static void draw_stats() {
  ...
  if (battle_drawing == 0) {
    ...
    sprintf(buffer, "Attack : %d ~ %d", poke_info[show_pokemon].attack_min,
            poke_info[show_pokemon].attack_max);
  ...
  if (battle_drawing == 1) {
    ...
    sprintf(buffer, "Attack : %d ~ %d", poke_info[favorite_pokemon].attack_min,
            poke_info[favorite_pokemon].attack_max);
    text_log(0, -1.8, buffer);
  ...
  if (battle_drawing == 2) {
    ...
    sprintf(buffer, "Attack : %d ~ %d", wild_pokemon_stats.attack_min,
            wild_pokemon_stats.attack_max);
    text_log(0, -1.8, buffer);
...
static void draw_pokemons() {
  ...
  if (battle_drawing == 0) {
    glTranslatef(-5, 7, 4);
    glScalef(10, 10, 1);
    if (owned_pokemons.count(show_pokemon) || show_pokemon == 0 ||
        show_pokemon == 1 || show_pokemon == 2) {
...
static auto check_proximity(float x1, float y1, float w1, float h1, float x2,
                            float y2, float w2, float h2) -> bool {
  if (check_collision(x1 + .1, y1, w1, h1, x2, y2, w2, h2))
    return true;

  if (check_collision(x1 - .1, y1, w1, h1, x2, y2, w2, h2))
    return true;

  if (check_collision(x1, y1 + .1, w1, h1, x2, y2, w2, h2))
    return true;
...
void keyboard(unsigned char key, int x, int y) {
  ...
  if (window_select == WINDOW_FIELD) {
    ...
    case 'W':
      if (player_info.y <= 10) {
        if (!check_collision(player_info.x, player_info.y + .1,
                             player_info.width, player_info.height,
                             pokecenter_info.x, pokecenter_info.y,
                             pokecenter_info.width, pokecenter_info.height)) {
          if (check_collision(
                  player_info.x, player_info.y + .1, player_info.width,
                  player_info.height, wild_pokemon_info.x, wild_pokemon_info.y,
                  wild_pokemon_info.width, wild_pokemon_info.height)) {
            init_battle();
          }
    ...
    case 'A':
      if (player_info.x >= -10) {
        if (!check_collision(player_info.x - .1, player_info.y,
                             player_info.width, player_info.height,
                             pokecenter_info.x, pokecenter_info.y,
                             pokecenter_info.width, pokecenter_info.height)) {
          if (check_collision(
                  player_info.x - .1, player_info.y, player_info.width,
                  player_info.height, wild_pokemon_info.x, wild_pokemon_info.y,
                  wild_pokemon_info.width, wild_pokemon_info.height)) {
            init_battle();
          }
    ...
    case 'h':
    case 'H': {
      if (check_proximity(player_info.x, player_info.y, player_info.width,
                          player_info.height, pokecenter_info.x,
                          pokecenter_info.y, pokecenter_info.width,
                          pokecenter_info.height)) {
        write_message = true;
